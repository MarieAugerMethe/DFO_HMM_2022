...
```{r import_depth_data, warning=FALSE}
...
```{r regularise_depth_data, warning=FALSE}
# regularise data 
dives <- dives %>% 
  group_by(ID) %>%
...
```{r convert_depth_data_to_TDR, warning=FALSE}
...
```{r calibrate_TDR, warning=FALSE, cache=TRUE}
...
```{r summarise_dive_data_to_10 min}
dives_sum <- dives_df %>% 
...
```{r prep_dive_data_and_independent_HMMs, message=FALSE}
...
```{r 3_state_dive_and_move_HMM, message=FALSE, cache=TRUE}
# define states
