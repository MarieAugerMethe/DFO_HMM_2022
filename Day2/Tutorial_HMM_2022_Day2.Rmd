...
<!-- MAM: to be able to run the code in the .Rmd  you need to put all the code in the chunks. For it to work here when compiling, we need to have eval=FALSE (since the default wd for compiling is the directory with .Rmd file.)  -->
...
<!-- MAM: If you only use the August data throughout, create a file that has just the August data.  -->
<!-- MAM: if we go ahead and drop the Argos data, also remove the Argos data all together from this file, and just present teh GPS data. The less steps we have the more they will learn the real stuff. -->
...
```{r import tracks, message=FALSE, attr.source = ".numberLines"}
...
```{r view_track_dt, attr.source = ".numberLines"}
...
2. Nullification <!-- MAM: I find this term weird. -->
3. Path segmentation
...
<!-- MAM: is the term nullify something you saw somewhere? I'm not sure it's very intuitive. -->
...
```{r id_nullify_gaps, attr.source = ".numberLines"}
...
```{r nullified_crawl_HMM_fitHMM, message=FALSE, attr.source = ".numberLines"}
set.seed(1)
# Setting up the starting values
mu0 <- c(50, 500) # Mean step length
sigma0 <- c(50, 500) # Sd of the step length
...
```{r prep_dive_data_and_independent_HMMs, message=FALSE, attr.source = ".numberLines"}
...
<!--   prior = prior,  -->
<!--   predTime = predTime)  -->
<!-- plot(argos_45_crwOut, ask = FALSE) -->
...
<!-- <!-- MAM: I get some pretty daming error message here. --> -->
...
<!-- ```{r nullified_crawl_HMM_fitHMM_argos, message=FALSE} -->
<!-- set.seed(1) -->
<!-- # Fit a 2 state HMM for 45 min ARGOS and GPS tracks -->
