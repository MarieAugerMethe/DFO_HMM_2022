...
```{r linear_HMM_prep, attr.source = ".numberLines"}
...
```{r linear_HMM_fitHMM, message=FALSE, attr.source = ".numberLines"}
set.seed(1)
# Fit a 2 state HMM
...
set.seed(1) # crawl can fail to fit periodically, so I recommend always setting a seed 
# fit crawl
...
<!-- MAM: I'm getting warning messages here. I wonder if it's because I removed the adding 10 sec (which I assumed was more for Argos data. If you add back the adding 10 sec, consider adding just 1 sec, and explain it well. -->
...
```{r crawl_HMM_fitHMM, message=FALSE, attr.source = ".numberLines"}
set.seed(1)
# prep data 
...
```{r voided_crawl_HMM_fitHMM, message=FALSE, attr.source = ".numberLines"}
set.seed(1)
# Setting up the starting values
mu0 <- c(50, 500) # Mean step length
sigma0 <- c(50, 500) # Sd of the step length
...
```{r define_error_model, attr.source = ".numberLines"}
# define error model
...
```{r define_argos_priors, message=FALSE, eval = T, attr.source = ".numberLines"}
# define priors
prior <- function(p) {
...
```{r fit_45_min_crawl_to_argos_tracks, message=FALSE, cache=TRUE, attr.source = ".numberLines"}
...
```{r fit_45_min_crawl_to_gps_tracks, message=FALSE, attr.source = ".numberLines"}
...
```{r prep_ARGOS_tracks_and_fit_basic_HMM, message = FALSE, cache=T, attr.source = ".numberLines"}
# prep tracks
...
```{r prep_data_and_void_gaps, attr.source = ".numberLines"}
...
```{r voided_crawl_HMM_fitHMM_argos, message=FALSE, attr.source = ".numberLines"}
set.seed(1)
# Fit a 2 state HMM for 45 min ARGOS and GPS tracks
...
```{r subset_argos_T172062, attr.source = ".numberLines"}
# subset 
...
```{r define_sub_argos_priors, message=FALSE, attr.source = ".numberLines"}
# define priors
prior <-  function(p) {
...
```{r calculate_sub_argos_fix_par, message=FALSE, attr.source = ".numberLines"}
...
```{r calculate_define_sub_argos_priors, message=FALSE, eval=TRUE, cache=T, attr.source = ".numberLines"}
# fit
set.seed(1)
...
```{r MIfitHMM_argos_priors, message=FALSE, cache=T, attr.source = ".numberLines"}
...
```{r calc-summary_dive_stats, attr.source = ".numberLines"}
...
```{r data_structure, attr.source = ".numberLines"}
...
```{r prep_dive_data_and_independent_HMMs, message=FALSE, attr.source = ".numberLines", cache=T}
...
```{r view_HMM_move_dive, attr.source = ".numberLines"}
...
```{r review_3_state_HMM, attr.source = ".numberLines"}
...
<!-- ``` -->
## Modelling emission probabilities
## Bonus 
<!-- ### Dealing with location error and irregular time -->
<!-- Thus far, we have been using exclusively the GPS data, which can be assumed to have a negligible error in most applications. However, in many marine systems, we may obtain ARGOS locations, which can have highly variable location error on the order of hundreds of meters to hundreds of km. In addition, ARGOS data often provides location estimates in very irregular time intervals. `crawlWrap` can be used not only to predict impute missing data, but also to predict location when faced with high location error and irregular time-series. -->
